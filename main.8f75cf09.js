...
var bestProductFrame = document.querySelector('.best_product > .product_frame');
var productUl01 = document.querySelector('.p_l_01');
var productUl02 = document.querySelector('.p_l_02');
for (var i = 0; i < _data.default.length; i++) {
  ...
  //span에 들어가는 text
  var bestLiTextModelText = document.createTextNode(_data.default[i].modelName);
  var bestLiTextNameText = document.createTextNode(_data.default[i].name);

  //이미지 속성 변경
  bestLiInImg.setAttribute('src', _data.default[i].src);
  ...
  if (i < _data.default.length / 2) {
    productUl01.appendChild(addBestLi);
  } else if (i >= _data.default.length / 2) {
...
var prevBtn = document.querySelector('.btn01');
var nextBtn = document.querySelector('.btn02');
var bestLi = document.querySelectorAll('.Best_product_list');

//ul width 추출
var UlWidth = productUl01.offsetWidth;
